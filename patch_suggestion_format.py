...
def _judge_reply_with_llm(original_finding, reply_body):
    ...
    try:
        ...
        system_prompt = (
            ...
            "narrow cases above.\n"
            "- If the reply cites an existing mitigation or notes the concern is already "
            "addressed elsewhere in the PR, treat the cited mitigation at face value "
            "(the classifier only sees the finding + reply, not the code, so do NOT try "
            "to independently verify the mitigation) and prefer ACCEPT.\n"
