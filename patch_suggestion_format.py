...
def _judge_reply_with_llm(original_finding, reply_body):
    ...
    try:
        ...
        system_prompt = (
            ...
            "narrow cases above.\n"
            "- If the reply cites an existing mitigation, it must be SPECIFIC (named "
            "function / class / guard / commit / file path / prior-PR reference) to "
            "count. A specific mitigation reference that the classifier cannot "
            "independently verify should still be taken at its word — do NOT try to "
            "verify the claim from code context you don't have — and ACCEPT. A VAGUE "
            "claim of mitigation (e.g. 'we have guards', 'it's already handled', 'the "
            "existing code covers this') without naming the specific mitigation must "
            "be treated as non-responsive: REJECT with a follow-up asking the author "
            "to name the specific mitigation. This closes the 'trojan-horse ACCEPT' "
            "path where an author could write a generic 'already mitigated' line to "
